...
class LLMBatch:
    ...
    def prompt_composer(
        ...
        return """You are a precise geolocation classifier.
You will receive a Twitter user’s self-filled location text.  
Your task is to determine whether the user is indicating a geographical location,  
and if so, whether that location is in EUROPE (geographical Europe, not political EU).
Reply with only one number:
0 = cannot tell / ambiguous / not a valid location
1 = possibly/definitely in Europe
2 = possibly/definitely NOT in Europe
...
- You should try to infer the intended location *only if* there is a plausible, geographically grounded interpretation.
- If the entry does not point to a meaningful location, output 0.
...
if __name__ == "__main__":
    batch = LLMBatch(working_dir="eu_gpt_analysis", mode="prod")
    batch.process()
